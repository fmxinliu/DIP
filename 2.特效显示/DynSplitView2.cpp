...
BEGIN_MESSAGE_MAP(CDynSplitView2, CView)
    ...
    ON_COMMAND(ID_RedToBlack, &CDynSplitView2::OnRedToBlack)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{     
    ...
    if(!pDoc ->statedoc&&state2==1)
    {
        ...
        else
        {
            ...
            {
                ::StretchDIBits(pDC->GetSafeHdc(),0, j, bitmapWidth, 1,
                    0, bitmapHeight-j-1, bitmapWidth, 1,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                Sleep(5);
            }
    ...
    if(!pDoc ->statedoc&&state2==1)
    {
        ...
        else
        {
            ...
            for (int x = 0; x < bitmapWidth; x++) {
                ::StretchDIBits(pDC->GetSafeHdc(), 0, 0, x, bitmapHeight,
                    bitmapWidth - x - 1, 0, x, bitmapHeight,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                Sleep(5);
            }
    ...
    if(!pDoc ->statedoc&&state2==1)
    {
        ...
        else
        {
            ...
            for (int y = bitmapHeight - 1; y >= 0; y--) {
                ::StretchDIBits(pDC->GetSafeHdc(), 0, y, bitmapWidth, bitmapHeight - y - 1,
                    0, y, bitmapWidth, bitmapHeight - y - 1,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                Sleep(5);
            }
...
void CDynSplitView2::OnJiaoChaFeiRu() 
{
    ...
    if(!pDoc ->statedoc&&state2==1)
    {
        ...
        else
        {
            ...
            // 快
            for (int x = 0; x < bitmapWidth; x++) {
                ::StretchDIBits(pDC->GetSafeHdc(), 0, 0, x, bitmapHeight/2,
                    bitmapWidth-x-1, bitmapHeight/2, x, bitmapHeight/2,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                ::StretchDIBits(pDC->GetSafeHdc(), bitmapWidth-x-1, bitmapHeight/2, x, bitmapHeight/2,
                    0, 0, x, bitmapHeight/2,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                Sleep(10);
            }
            ...
            //        ::StretchDIBits(pDC->GetSafeHdc(), 0, y, x, 1,
            //            bitmapWidth-x-1, bitmapHeight - y - 1, x, 1,
            //            pBitmapData, pBitmapInfo,
            //            DIB_RGB_COLORS, SRCCOPY);
            //        ::StretchDIBits(pDC->GetSafeHdc(), bitmapWidth-x-1, bitmapHeight/2, x, bitmapHeight/2,
            //            0, 0, x, bitmapHeight/2,
...
void CDynSplitView2::OnZhongJianKuoZhang() 
{
    ...
    {         
        ...
        {
            for (int y = 0; y < bitmapHeight / 2; y++) {
                ::StretchDIBits(pDC->GetSafeHdc(), 0, bitmapHeight / 2 - y, bitmapWidth, 1,
                    0, bitmapHeight / 2 + y -1, bitmapWidth, 1,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                ::StretchDIBits(pDC->GetSafeHdc(), 0, bitmapHeight / 2 + y, bitmapWidth, 1,
                    0, bitmapHeight / 2 - y -1, bitmapWidth, 1,
...
void CDynSplitView2::OnZhongJianShouSuo() 
{
    ...
    if(!pDoc ->statedoc&&state2==1)
    {
        ...
        {
            for (int y = 0; y < bitmapHeight / 2; y++) {
                ::StretchDIBits(pDC->GetSafeHdc(), 0, y, bitmapWidth, 1,
                    0, bitmapHeight - y - 1, bitmapWidth, 1,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                ::StretchDIBits(pDC->GetSafeHdc(), 0, bitmapHeight - y - 1, bitmapWidth, 1,
                    0, y, bitmapWidth, 1,
                    pBitmapData, pBitmapInfo,
                    DIB_RGB_COLORS, SRCCOPY);
                Sleep(5);
...
void CDynSplitView2::OnRedToBlack()
{
    //刷新屏幕
    CDC *pDC=GetDC();
    CRect rect(0,0,1000,1000);
    ...
    if(!pDoc ->statedoc&&state2==1)
    {
        ...
        int bitmapHeight = CDibNew1->GetHeight();
        int bitmapWidth = CDibNew1->GetWidth();
        LONG widthBytes = CDibNew1->LineBytes();
        ...
        else
        {
            for (int y = 0; y < bitmapHeight; y++) {
                for (int x = 0; x < bitmapWidth; x++) {
                    int index = (bitmapHeight - 1 - y) * widthBytes + x * 3;
                    if (pBitmapData[index] == 0 &&  // B
                        pBitmapData[index + 1] == 0 && // G
                        pBitmapData[index + 2] == 255) { // R
                        pBitmapData[index + 2] = 0;
                    }
                }
            }

            ::StretchDIBits(pDC->GetSafeHdc(), 0, 0, bitmapWidth, bitmapHeight,
                0, bitmapHeight - 1, bitmapWidth, bitmapHeight,
                pBitmapData, pBitmapInfo,
                DIB_RGB_COLORS, SRCCOPY);
        }
    }
