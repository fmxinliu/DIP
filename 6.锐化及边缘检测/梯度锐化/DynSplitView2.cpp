...
BEGIN_MESSAGE_MAP(CDynSplitView2, CView)
    ...
    ON_COMMAND(ID_TiduDaiti, &CDynSplitView2::OnTidudaiti)
    ON_COMMAND(ID_GuDingYuZhi2, &CDynSplitView2::OnGudingyuzhi2)
    ON_COMMAND(ID_RgbToGray, &CDynSplitView2::OnRgbtogray)
    ON_COMMAND(ID_Threshold, &CDynSplitView2::OnThreshold)
    //}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
      clearmem();
    CDibNew1->Menxianruihua(30);     //对图像进行门限锐化
    Invalidate();    
}
...
{
    clearmem();
    CDibNew1->Erzhirihua(30);     //对图像进行二值锐化
    Invalidate();
}
...
{
    clearmem();
    CDibNew1->GuDingRuiHua(30, false);     //对图像进行固定阈值锐化
    Invalidate();
}


void CDynSplitView2::OnGudingyuzhi2()
{
    clearmem();
    CDibNew1->GuDingRuiHua(30, true);     //对图像进行直接梯度值代替
    Invalidate();
}

void CDynSplitView2::OnTidudaiti()
{
    clearmem();
    CDibNew1->TiduzhiDaiti();     //对图像进行直接梯度值代替
    Invalidate();
}
...
{
    clearmem();
    CDibNew1->RgbToGray();     // 彩色图像转灰度
    //CDibNew1->RgbToGrayFast();
    Invalidate();
}


void CDynSplitView2::OnThreshold()
{
    clearmem();
    CDibNew1->Threshold();     // 二值化
    Invalidate();
}
