...
void CDynSplitView2::OnXuanzhuan() 
{
    ...
    // 获取用户设定的旋转量
    CDibNew1->Xuanzhuan(dlg.m_iRotateAngle);
    this->Invalidate();
}
...
void CDynSplitView2::OnFangda() 
{
    ...
    CDibNew1->FangDa(xzoom, yzoom);
    //CDibNew1->m_pData = CDibNew1->FangDa(xzoom, yzoom);
    //CDibNew1->SetWidth(CDibNew1->GetWidth() * xzoom);
    //CDibNew1->SetHeight(CDibNew1->GetHeight() * yzoom);
    Invalidate();
    return;
    ...
    if(CDibNew1->m_pBitmapInfoHeader->biBitCount<9)
    {
        ...
        temp =CDibNew1->FangDa(temp,xzoom,yzoom,wide,height,newLineBytes,newheight); // bmp头与像素不连续!!! 
        CDibNew1->m_pData = temp;
