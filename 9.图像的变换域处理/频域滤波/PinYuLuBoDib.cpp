...
CNumber PinYuLuBoDib::Add(const CNumber &c1, const CNumber &c2)
{
    CNumber c;
    c.re = c1.re + c2.re;
    c.im = c1.im + c2.im;
...
CNumber PinYuLuBoDib::Sub(const CNumber &c1, const CNumber &c2)
{
    CNumber c;
    c.re = c1.re - c2.re;
    c.im = c1.im - c2.im;
...
CNumber PinYuLuBoDib::Mul(const CNumber &c1, const CNumber &c2)
{
    CNumber c;
...
{
    LPBYTE  p_data, p;//指向原图像数据区指针
    int width,height;//原图像的宽度和高度       
    ...
    if(this->byBitCount==8)    //灰度图像
        p_data=this->GetData();//指向原图像数据区
    else    //24位真彩色
        p_data=this->GetData2();
    width=this->GetWidth();//得到图像宽度
    height=this->GetHeight();//得到图像高度
    ...
    for(j=0;j<h;j++)
    {
        for(i=0;i<w;i++)
        {
            p=p_data+lLineBytes*(height-j-1)+i;//指向第i行第j列像素
            t[i+w*j].re=*(p);//给时域赋值
            t[i+w*j].im=0;
        }
    }
...
///////////////////////////////////////////////
//此函数用来实现图像的傅立叶变换(没有对处理后的显示结果进行平移)
//两次调用快速傅立叶变换QFC()实现二维傅立叶变换
///////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////
//该函数用来实现二维傅立叶变换
//参数height、width分别表示图像的高度和宽度，isign表示正反变换
//////////////////////////////////////////////////////////////
...
*参数：u、v分别是截止频率的x、y分量值，n是滤波阶数，由用户给定
*功能：此函数用来实现图像的布特沃斯低通滤波
/*************************************************************************/
...
////////////////////////////////////////////////
//此函数用来实现图像的布特沃斯高通滤波
//参数u、v分别是截止频率的x、y分量值，n是滤波阶数，由用户给定
////////////////////////////////////////////////
 ...
 void PinYuLuBoDib::PerfectFilterL(int u,int v)
{
    ...
    fourier(t,height,lLineBytes,1); /// 傅里叶正变换
    ...
    fourier(t,height,lLineBytes,-1); /// 傅里叶反变换
...
*参数：u、v分别是截止频率的x、y分量值，n是衰减系数，由用户给定
*功能：此函数用来实现图像的指数低通滤波
///////////////////////////////////////////////*/
...
////////////////////////////////////////////////
//此函数用来实现图像的指数高通滤波
//参数u、v分别是截止频率的x、y分量值，n是衰减系数，由用户给定
////////////////////////////////////////////////
